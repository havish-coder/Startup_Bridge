...
export default function Negotiations() {
  ...
  async function fetchNegotiations() {
    try {
      const res = await api.get('/startup/negotiations')
      setData(res.data)
    } catch (err) {
      console.error(err)
  ...
  return (
    <Layout>
      {/* Header */}
      <div className="mb-8">
        <h2 className="text-2xl font-bold text-gray-900 tracking-tight">Active & Concluded Deals</h2>
        <p className="text-gray-500 mt-1">Track final terms and statuses of negotiations accepted by you.</p>
      </div>
      ...
      {data.length === 0 ? (
        <Card className="text-center py-12 max-w-lg mx-auto mt-6">
          ...
          <p className="text-gray-600 font-semibold text-lg">No deal negotiations found</p>
          <p className="text-sm text-gray-400 mt-1">Once you accept an investor's proposal, the negotiation deal details will populate here.</p>
          <Button className="mt-5" onClick={() => navigate('/startup/pitches')}>
            View My Pitches
          </Button>
        </Card>
      ) : (
                  ...
                  {n.acceptedInvestor && (
                    <div className="pt-2 text-xs text-gray-600 space-y-1">
                      <div className="flex items-center gap-1.5">
                        <span className="font-semibold text-gray-800">Accepted Investor:</span>
                        <span>{n.acceptedInvestor.name} ({n.acceptedInvestor.email})</span>
                      </div>
                      {n.acceptedInvestor.contactPhone && (
                        <div className="flex items-center gap-1.5">
                          <span className="font-semibold text-gray-800">Contact Phone:</span>
                          <span>{n.acceptedInvestor.contactPhone}</span>
                        </div>
                      )}
                    </div>
                  )}
                ...
                {n.finalAmount && n.finalEquityPct ? (
                  ...
                  <div className="text-left sm:text-right bg-amber-50/50 p-4 rounded-xl border border-amber-100 sm:min-w-[200px]">
                    ...
                    <p className="text-xs font-semibold text-amber-800 mt-1.5">
                      Awaiting initial proposal acceptance
                    </p>
                  </div>
                )}
              </div>
